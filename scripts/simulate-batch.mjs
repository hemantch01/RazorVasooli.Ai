 ...
 *
 * Usage:
 *   node scripts/simulate-batch.mjs [--seed 424242] [--size 45] [--both]
 */
...
async function runBatch(agentOn) {
  ...
  const b = data.batch;
  console.log(
    `✅ ${b.label}: ${b.size} txns · recovered ₹${b.summary.totalRecovered.toLocaleString("en-IN")} · rate ${(b.summary.recoveryRate * 100).toFixed(1)}%`
  );
}
...
try {
  ...
  const rep = await (await fetch(`${API}/api/simulator/ab-report`)).json();
